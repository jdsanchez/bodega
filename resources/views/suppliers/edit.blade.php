                ...
                <div class="p-6">
                    <form action="{{ route('suppliers.update', $supplier) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-6">
                            ...
                            <input type="text" name="nit" id="nit" value="{{ old('nit', $supplier->nit) }}" required
                        ...
                        <div class="mb-6">
                            ...
                            <div class="flex items-center space-x-4">
                                <div class="flex-shrink-0">
                                    @if($supplier->logo)
                                        <img id="logoPreview" src="{{ Storage::url($supplier->logo) }}" alt="Logo actual" 
                                    ...
                                    @endif
                                    <div id="logoPlaceholder" class="w-24 h-24 rounded-lg bg-gradient-to-br from-indigo-400 to-indigo-600 flex items-center justify-center text-white text-2xl font-bold {{ $supplier->logo ? 'hidden' : '' }}">
                                ...
                                <div class="flex-grow">
                                    ...
                                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                        {{ $supplier->logo ? 'Dejar vacío para mantener el logo actual.' : 'Formatos permitidos: JPG, PNG, GIF. Máximo 2MB.' }}
                                    </p>
                                    @error('logo')
                                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                    @enderror
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="address" id="address" rows="3"
                                      class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-gray-100 @error('address') border-red-500 @enderror">{{ old('address', $supplier->address) }}</textarea>
                            @error('address')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            ...
                            <div>
                                ...
                                <div class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-100 dark:bg-gray-900 text-gray-700 dark:text-gray-300">
                                    {{ $supplier->registration_date->format('d/m/Y H:i') }}
                                </div>
                                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                    {{ $supplier->registration_date->diffForHumans() }}
                                </p>
                            </div>
                        </div>
